...
`.trim()
    conn.tebakkabupaten[id] = [
        await conn.sendButtonImg(m.chat, await (await fetch(json.url)).buffer(), caption, 'BRAYY-BOTZ`', 'Bantuan', '.teka', m),
        json, poin,
        setTimeout(async () => {
            if (conn.tebakkabupaten[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.title}*`, 'BRAYY-BOTZ`', 'Tebak Kabupaten', '.tebakkabupaten', conn.tebakkabupaten[id][0])
